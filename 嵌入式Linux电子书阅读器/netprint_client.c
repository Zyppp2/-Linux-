...
int main(int argc, char **argv)
{
	...
	tSocketServerAddr.sin_port        = htons(SERVER_PORT);  /* host to net, short */
 	...
 	if (0 == inet_aton(argv[1], &tSocketServerAddr.sin_addr)) /*将提供的IP地址从字符串格式转换为二进制格式并将其存储在tSocketServerAddr.sin_addr中*/
 	{
	...
	if (strcmp(argv[2], "show") == 0)
	{
		...
		iSendLen = sendto(iSocketClient, "setclient", 9, 0,                    /*将消息 "setclient" 发送到服务器*/
