...
static int isUtf8Coding(unsigned char *pucBufHead)         /* 检查文字是不是UTF-8编码 */
{
...
static int Utf8GetCodeFrmBuf(unsigned char *pucBufStart, unsigned char *pucBufEnd, unsigned int *pdwCode)
{
	...
	else                                        /* 这段代码就是在写unicode的编码组成规则，简单来说首先看开头1的个数，如果为0则确定为acsii码  */  
	{                                           /* 如果为1则代表用一个字节表示该字符，如果为2则用两个字节表示，以此类推  */
		ucVal = ucVal >> iNum;                  /* 然后将第一个字节除开头1之外的数字留存，若为1个以上字节表示则留存后面字节的6位依次累加  */
		ucVal = ucVal << iNum;                  /* 具体搜索Unicode编码规则 */   
		dwSum += ucVal;
		for (i = 1; i < iNum; i++)
		{
