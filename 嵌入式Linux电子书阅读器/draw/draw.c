...
int OpenTextFile(char *pcFileName)               /* 打开文本文件 */
{
	struct stat tStat;
	...
	g_pucTextFileMemEnd = g_pucTextFileMem + tStat.st_size;              /*确定文件内存缓冲区的结束位置*/
	
	g_ptEncodingOprForFile = SelectEncodingOprForFile(g_pucTextFileMem); /* 找到文本文件对应的字体编码操作器位置*/
...
{
	int iError = 0;                                                        /*遍历已知的字体操作链表，并用相应的字体初始化函数来初始化字体*/
...
int SelectAndInitDisplay(char *pcName)        /*根据输入的显示设备名称选择对应的显示设备操作结构体，并初始化显示设备*/
{
	int iError;
...
int IncLcdX(int iX)                         /*用来递增 LCD 显示的 X 坐标*/
{
...
int IncLcdY(int iY)						   /*用来递增 LCD 显示的 Y 坐标*/
{
...
int RelocateFontPos(PT_FontBitMap ptFontBitMap)   /*更新定位字符的位置*/
{
	int iLcdY;
	int iDeltaX;
	int iDeltaY;
...
int ShowOneFont(PT_FontBitMap ptFontBitMap)     /*绘制字体*/
{
	int x;
	int y;
	...
	if (ptFontBitMap->iBpp == 1)
	{
		...
		{
			i = (y - ptFontBitMap->iYTop) * ptFontBitMap->iPitch;  /*每行像素数据的字节数，bpp为1，每个像素只占用一个位，因此需要确保在读取每个像素时正确地跳过填充字节或间隙。*/
	...
	else if (ptFontBitMap->iBpp == 8)								/*bpp为8，每个像素占据一个字节，字体位图的每一行的数据是连续存储的，不需要ipitch确定位置*/
	{
		for (y = ptFontBitMap->iYTop; y < ptFontBitMap->iYMax; y++)
...
static void RecordPage(PT_PageDesc ptPageNew)    /*记录页面*/
{
	PT_PageDesc ptPage;
